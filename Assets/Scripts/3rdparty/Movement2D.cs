...
public class Movement2D : MonoBehaviour
{
    ...
    public void ForceMoveAlongX(float distance)
    {
        motion = Vector3.zero;
        motion.x = distance;
        MoveGameObjectToPosition(myTransform.TransformPoint(motion) - myTransform.position);
    }
    ...
    public void ForceMoveAlongY(float distance)
    {
        motion = Vector3.zero;
        motion.y = distance;
        MoveGameObjectToPosition(myTransform.TransformPoint(motion) - myTransform.position);
    ...
    public bool CanMoveTo(Vector2 motion, ref RaycastHit2D[] hits)
    {
        if (shapeMode == ShapeMode.Circle)
            hits = Physics2D.CircleCastAll(myTransform.position + (Vector3) offsetFromPivot, radius, motion,
                motion.magnitude, layerMask);
        else if (shapeMode == ShapeMode.Box)
            hits = Physics2D.BoxCastAll(transform.position + (Vector3) offsetFromPivot, (Vector3) size, 0.0f, motion,
                motion.magnitude, layerMask);
    ...
    public bool CanMoveTo(Vector2 motion, ref Vector2 safeMotion, ref RaycastHit2D[] hits)
    {
        if (shapeMode == ShapeMode.Circle)
            hits = Physics2D.CircleCastAll(
                myTransform.position + new Vector3(offsetFromPivot.x, offsetFromPivot.y, 0.0f), radius, motion,
                motion.magnitude, layerMask);
        else if (shapeMode == ShapeMode.Box)
            hits = Physics2D.BoxCastAll(transform.position + new Vector3(offsetFromPivot.x, offsetFromPivot.y, 0.0f),
                new Vector3(size.x, size.y, 0.0f), 0.0f, motion, motion.magnitude, layerMask);
        foreach (RaycastHit2D hit in hits)
        {
            if (hit.transform.GetInstanceID() != myTransform.GetInstanceID())
            {
                safeMotion =
                ((myTransform.position - (Vector3) hit.point).magnitude -
                 ((shapeMode == ShapeMode.Circle) ? radius : Mathf.Max(size.x, size.y))) * motion.normalized;
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            Gizmos.DrawWireCube(transform.position + new Vector3(offsetFromPivot.x, offsetFromPivot.y, 0.0f),
